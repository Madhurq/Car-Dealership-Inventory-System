...
public interface VehicleRepository extends JpaRepository<Vehicle, Long> {
    ...
    @Query("SELECT v FROM Vehicle v WHERE " +
           "(:make IS NULL OR LOWER(v.make) LIKE LOWER(:make)) AND " +
           "(:model IS NULL OR LOWER(v.model) LIKE LOWER(:model)) AND " +
           "(:category IS NULL OR LOWER(v.category) LIKE LOWER(:category)) AND " +
